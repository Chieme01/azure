...
output "storage_account_name" {
  value = azurerm_storage_account.k8-devops-sa.name
}

output "container_name" {
  value = azurerm_storage_container.k8-devops-container.name
}
...
output "master_node_identity_principal_id" {
  value = [for v in azurerm_linux_virtual_machine.all_nodes : v.identity[0].principal_id] 
}
